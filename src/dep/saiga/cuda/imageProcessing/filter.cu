...
void applyFilterSeparate(SiftImageType src, SiftImageType dst, SiftImageType tmp, array_view<float> kernelRow, array_view<float> kernelCol){
    convolveRow(src,tmp,kernelRow,kernelRow.size() / 2);
    convolveCol(tmp,dst,kernelCol,kernelCol.size() / 2);
}

void applyFilterSeparateSinglePass(SiftImageType src, SiftImageType dst, array_view<float> kernel){
